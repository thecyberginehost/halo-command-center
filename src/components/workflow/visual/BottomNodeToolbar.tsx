...
export function BottomNodeToolbar({ onAddNode }: BottomNodeToolbarProps) {
  ...
  const NodeItem = ({ integration }: { integration: IntegrationNode }) => {
    ...
    return (
      <div
        ...
        className="flex items-center space-x-3 p-3 rounded-lg cursor-grab hover:bg-accent/50 transition-colors border hover:border-primary/20"
        onClick={() => {
      ...
      >
        <div 
          className="p-2 rounded flex items-center justify-center min-w-[32px]"
  ...
  return (
    <Card className="fixed bottom-4 left-1/2 transform -translate-x-1/2 z-50 bg-background/95 backdrop-blur-sm border shadow-lg">
      <div className="flex items-center gap-2 p-3">
        ...
        <div className="flex items-center gap-1">
          {nodeTypeGroups.map((group) => {
            const integrations = getIntegrationsForGroup(group);
