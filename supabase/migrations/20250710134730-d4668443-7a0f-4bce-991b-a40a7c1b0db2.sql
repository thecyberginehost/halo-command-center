-- Phase 4: Enterprise Features Database Schema (Fixed)
...
CREATE INDEX idx_tenant_features_tenant_feature ON public.tenant_features(tenant_id, feature_name);
CREATE INDEX idx_tenant_quotas_tenant_resource ON public.tenant_quotas(tenant_id, resource_type);
CREATE INDEX idx_marketplace_packages_category ON public.marketplace_packages(category, is_active);
CREATE INDEX idx_marketplace_packages_vendor ON public.marketplace_packages(vendor_name, is_active);
CREATE INDEX idx_tenant_marketplace_installs_tenant ON public.tenant_marketplace_installs(tenant_id, is_active);
CREATE INDEX idx_marketplace_reviews_package ON public.marketplace_reviews(package_id, is_visible);
CREATE INDEX idx_execution_metrics_tenant_workflow ON public.execution_metrics(tenant_id, workflow_id, recorded_at);
CREATE INDEX idx_execution_metrics_execution_step ON public.execution_metrics(execution_id, step_id);
CREATE INDEX idx_execution_metrics_type_time ON public.execution_metrics(metric_type, recorded_at);
CREATE INDEX idx_performance_benchmarks_tenant_category ON public.performance_benchmarks(tenant_id, benchmark_type);
CREATE INDEX idx_performance_alerts_tenant_severity ON public.performance_alerts(tenant_id, severity, is_resolved);
CREATE INDEX idx_performance_alerts_created_resolved ON public.performance_alerts(created_at, is_resolved);
CREATE INDEX idx_integration_migrations_tenant_status ON public.integration_migrations(tenant_id, migration_status);
CREATE INDEX idx_integration_migrations_batch_status ON public.integration_migrations(migration_batch_id, migration_status);
CREATE INDEX idx_workflow_conversions_migration ON public.workflow_conversions(migration_id, conversion_status);
