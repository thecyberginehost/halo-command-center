...
echo -e "${YELLOW}🎮 Self-hosted automation platform for MASP-certified providers${NC}"
echo ""
...
if check_port "$DEFAULT_PORT"; then
    ...
    PORT=$(prompt_with_default "Enter alternative port:" "8081")
...
# Supabase Backend Configuration (update these with your actual keys)
...
if [[ "$PORT" != "8080" ]]; then
    ...
    sed -i "s/port: 8080/port: $PORT/" vite.config.ts
    sed -i "s/8080:8080/$PORT:$PORT/" docker-compose.yml
    sed -i "s/:8080/:$PORT/g" docker/nginx/conf.d/default.conf.template
fi
...
echo -e "${YELLOW}🎮 Port $PORT - Self-hosted automation platform${NC}"
echo ""
